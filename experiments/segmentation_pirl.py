...
from datasets.datasets import SegmentationDataSet, PIRLDataSet
...
def run(source_dir, target_dir, 
    ...
    source_val_dataset = SegmentationDataSet(root=target_dir, image_list_name=source_val_images, 
                            ...
                            mean=GTA5_MEAN, std=GTA5_STD, label2train=GTA5_LABELS2TRAIN)
    target_train_dataset = PIRLDataSet(root=target_dir, image_list_name=target_train_images, 
    ...
    for num_iter in range(start_epoch*len(source_train_dataloader), 
        ...
        aux_output_real, aux_output_perturbed = aux_output.split(target_train_dataloader.batch_size)
        aux_output_perturbed.squeeze_()
        aux_output_real.squeeze_()
        ...
        if negative_batch is not None:
            #  backprop aux_model
            tar_aux_loss = lambda_aux * aux_loss(aux_output_real, aux_output_perturbed, negative_batch.to(device))
            tar_aux_loss.backward()

        # dump image features in the memory bank
        memory_bank.update(aux_output_perturbed.detach())
